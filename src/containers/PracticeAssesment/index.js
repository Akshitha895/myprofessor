...
class PracticeAssesment extends Component {
  ...
  createtest() {
    ...
    console.log(
      'tthis.state.useDetails.userOrg',
      this.state.useDetails.userOrg
    );

    var userId = this.state.useDetails.userInfo.userId;

    var universityId = this.state.useDetails.userOrg.universityId;
    var branchId = this.state.useDetails.userOrg.branchId;
    var semesterId = this.state.useDetails.userOrg.semesterId;

    var subjectId = this.props.data.subjectId;

    var chapterId = this.props.data.chapterId;

    var userTestId = this.props.data.id;
    var url;
    var method;
    var body;
    if (this.props.attempttime === 1) {
      method = 'GET';
      console.log('dlknckldajckld', this.props.data.testType);
      if (
        this.props.data.testType === 'chapter' ||
        this.props.data.testType === 'Chapter'
      ) {
        url =
          baseUrl +
          `/universities/${universityId}/subjects/${subjectId}/practice-tests/${userTestId}/questions?userId=${userId}&testType=${this.props.data.testType}&chapterId=${chapterId}`;
      } else {
        url =
          baseUrl +
          `/universities/${universityId}/subjects/${subjectId}/practice-tests/${userTestId}/questions?userId=${userId}&testType=${this.props.data.testType}`;
        //  `/universities/${universityId}/subjects/${subjectId}/practice-tests?userId=${userId}`,
        console.log('dfmn,adjfnda', url);
      }
    } else {
      method = 'POST';
      ...
      url =
        baseUrl +
        `/universities/${universityId}/subjects/${subjectId}/practice-test/${this.props.data.testType}/re-attempt`;
      body = {
        userId,
        chapterId,
        testType: this.props.data.testType,
      };
      console.log('ksbfjdsBfjbdkfb,s', url);
    }
    console.log('sadsDSAdasdSADSADASdas', url);
    fetch(url, {
      method,
      headers: {
      ...
      .then((json) => {
        console.log('d.nckadjnck', json);
        if (json.code === 201) {
          //console.log("dknkasjkas" ,typeof json.data)
          var newarray = [];
          if (json.data.length > 0) {
            newarray = json.data;
            console.log('sdasDASDASDAS', newarray);
          } else {
            newarray = json.data.questions;
          ...
          if (newarray) {
            ...
            console.log('Dndknkdkcd', questions);
            this.setState(
              {
                getquestionsdata: newarray,
                finalarray: questions,
                testid: newarray[0].userTestId,
                seconds: newarray.length * 60,
                secondstime: newarray.length * 60,
              },
              () => {
                console.log('dnfcjadhfjdf', this.state.getquestionsdata);
                this.starttimer();
  ...
  onSubmit() {
    ...
    var url =
      baseUrl +
      `/universities/${universityId}/subjects/${subjectId}/practice-tests/${testId}/end`;
    console.log('dlcmkadmckd', url);
    this.setState(
      {
        isvisible: false,
      },
  ...
  getQuestionById() {
    ...
    this.setState({ loading: true });
    var testType = this.props.data.testType;
    ...
    console.log('Sdksklafdsf', this.state.getquestionsdata);
    console.log('dfsfff', questionindex);
    ...
    var url =
      baseUrl +
      `/universities/${universityId}/subjects/${subjectId}/practice-tests/${testId}/questions/${index}?questionId=${questionId}&userId=${userId}&testType=${testType}`;
              ...
              this.setState(
                {
                  loading: false,
                  spinner: false,
                  selectedItem: json.data[0] ? json.data[0] : {},
                },
                () => {
                  if (this.state.finalarray.length > 0) {
                    this.state.finalarray.map((res, i) => {
  ...
  validateAnswer(answerobj) {
    ...
    var url =
      baseUrl +
      `/universities/${universityId}/subjects/${subjectId}/practice-tests/${userTestId}/questions/${index}/validate`;
    var data = {
            ...
            >
              ...
              >
                ...
                >
                  ...
                  specific time to finish the test
                </Text>
                <Text
                  style={{
                    fontSize: 18,
