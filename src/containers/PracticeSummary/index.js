...
class PracticeSummary extends Component {
  ...
  getreport() {
    ...
    console.log('dm,ckdfkld', userTestId);
    var url =
      baseUrl +
      `/analytics/users/${userId}/assessments/${userTestId}/report?activityDimId=${undefined}`;
    ...
    fetch(url, {
      ...
      .catch((error) => console.log('gggg' + error));
  }
  logout() {
  ...
  onBack() {
    ...
    if (this.props.from === 'reviewscreen') {
      Actions.practicereview({ type: 'reset', data: this.props.data });
    } else {
      Actions.practicechapter({ type: 'reset', data: this.props.subjectData });
    }
  }
