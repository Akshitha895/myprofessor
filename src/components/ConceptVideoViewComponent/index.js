...
// var timesarray = [];
const ConceptVideoViewComponent = (props) => {
  ...
  onLoad = (data) => {
    console.log('onloaddd.......', normaldata);
    setDuration(parseInt(data.duration));
    if (normaldata.videoPausedAt > 1) {
  ...
  onEnd = (data) => {
    console.log('d.xcfdajfkhjdfkjdhfjd', data);
                  ...
                  <View style={styles.subright}>
                    {duration > 0 ? (
                      <Slider
                        ...
                        onValueChange={(value) => {
                          //  console.log("dkofjkoadfdf" , value  , currentTime)
                          if (parseInt(value) > parseInt(currentTime)) {
                            if (parseInt(props.resourcedata.progress) !== 0) {
                              // console.log("dafdfadfdfdfd", newarr)
...
export default ConceptVideoViewComponent;
